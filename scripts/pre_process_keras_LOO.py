...
# with open("../database/demographic_info.txt") as inf:
#     reader = csv.reader(inf, delimiter=" ")
#     patients = list(zip(*reader))[0]
# dem_file = open("../database/demographic_info.txt", 'r')
# lines = dem_file.readlines()

patients = [x.split(' ')[0] for x in open("../database/demographic_info.txt").readlines()]
print(patients)
...
for patient_num in patients:
    print("-----------", '\n', patient_num, '\n', "-----------")
    if ('\n' in patient_num): continue
    output_dir = "../database/" + patient_num + "/"
    ...
    for file in os.listdir(input_dir): #loop through all files in input directory
        if fnmatch.fnmatch(file, patient_num + '*'):
            # Is of patient number
            out_patient = True
        else: 
            out_patient = False
        ...
        if fnmatch.fnmatch(file, "*.wav"): #if its a WAV
            ...
            with open(input_dir + txt_file) as tsv: # open associated TXT file
                ...
                for line in csv.reader(tsv, delimiter="\t"): #iterate through each line of the text file
                    ...
                    if (out_patient):
                        sample_save_dir = output_dir + "testing" + '/sample_' + str(sample_count) + '.png'
                    else: sample_save_dir = output_dir + "training" + '/sample_' + str(sample_count) + '.png'

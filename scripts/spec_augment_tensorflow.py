...
# from SpecAugment import sparse_image_warp_np
...
def sparse_warp(mel_spectrogram, time_warping_para=2):
    ...
    warped_image, _ = sparse_image_warp(np.transpose(mel_spectrogram),
...
def visualization_spectrogram(mel_spectrogram):
    """visualizing first one result of SpecAugment

    # Arguments:
      mel_spectrogram(ndarray): mel_spectrogram to visualize.
      title(String): plot figure's titleS
    """
...
def visualization_tensor_spectrogram(mel_spectrogram, filename):
    """visualizing first one result of SpecAugment
    ...
    plt.imshow(mel_spectrogram, cmap=parula_map)
    plt.axis('off')
    plt.axes().get_xaxis().set_visible(False)
    plt.axes().get_yaxis().set_visible(False)
    fig=plt.gcf()
    plot_path = '../database/pre_processed_data_keras_augmented_butterworth/training'
    if "." in filename:
        Filename = filename.split(".")[0]
        plt.savefig(plot_path + '/' + Filename + '_aug' + '.png', bbox_inches='tight', pad_inches=0,)
        plt.clf()
        plt.show()